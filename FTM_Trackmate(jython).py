...
def process_directory(image_directory):
	...
	for f in files:
		...
		imp = IJ.openImage(f)
		imp.show()
		# process faster temporal median
		IJ.run(imp, "Use Opened Image and Run", "window=100 begin=1 end=1000 file=tif show output=[]")
		imp.show()
		
		# 100 nm pixels and 33 ms frame rate
		imp.getCalibration().setXUnit("nm");
		IJ.run(imp, "Properties...", "channels=1 slices=1 frames=1000 pixel_width=100 pixel_height=100 voxel_depth=100 frame=33ms");
